      ...
      {item.children.length === 0
        ...
        : (
          <div class="px-5 py-[22px]">
            <label
              class="flex justify-between items-center font-semibold	text-black text-base leading-[1.3125rem] tracking-[0]"
              htmlFor={`menu-item-checkbox-${index}`}
            >
              {item.text}
              <Icon
                id="chevron-right"
                class="fill-[#757575] text-[#757575]"
                size={36}
                fill={"#757575"}
              />
            </label>
            <input
              id={`menu-item-checkbox-${index}`}
              class="hidden peer"
              type="checkbox"
            />
            <div class="peer-checked:translate-x-0 translate-x-[-100%] transition-all duration-300 bg-white text-black fixed top-0 left-0 w-full h-full">
              <a
                class="w-full font-semibold tracking-[0] leading-[1.3125rem] text-primary text-base bg-base-200 flex items-center justify-center h-[70px]"
                href={item.url}
              >
                {item.text}
              </a>
              <label
                class="flex items-center gap-1.5 text-xs text-primary leading-3 tracking-[0.05rem] h-[38px] pl-1.5"
                htmlFor={`menu-item-checkbox-${index}`}
              >
                <Icon
                  id="chevron-right"
                  class="rotate-180 fill-[#757575] text-[#757575]"
                  size={30}
                  fill={"#757575"}
                />
                VOLTAR
                  ...
                  {leaf.links?.length === 0
                    ...
                    : (
                      <div class="px-5 py-[22px]">
                        <label
                          class="flex justify-between items-center font-semibold	text-black text-base leading-[1.3125rem] tracking-[0]"
                          htmlFor={`menu-item-checkbox-leaf-${index}-${indexLeaf}`}
                        >
                          {leaf.title?.text}
                          <Icon
                            id="chevron-right"
                        ...
                        <input
                          id={`menu-item-checkbox-leaf-${index}-${indexLeaf}`}
                          class="hidden peer"
                          type="checkbox"
                        />
                        <div class="peer-checked:translate-x-0 translate-x-[-100%] transition-all duration-300 bg-white text-black fixed top-0 left-0 w-full h-full">
  ...
  return (
    ...
    >
      ...
      <div class="bg-[#e3e3e3] text-primary text-xs leading-4 font-semibold uppercase flex justify-around h-[93px] items-center">
        <a
          href="/login"
          class="flex flex-col gap-2.5 justify-center items-center"
        >
          <Image
            class=""
